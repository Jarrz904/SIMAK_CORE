        ...
        <main class="flex-1 p-8">
            ...
            <div x-show="tab === 'dashboard'" x-transition x-cloak class="max-w-6xl mx-auto">
                            ...
                            <div class="flex items-center gap-2 mt-2">
                                <span class="text-slate-500 text-xs md:text-sm font-medium tracking-tight">
                                    @php
                                        ...
                                        $countLuarDaerah = $luarDaerahs->count(); // Tambahan variabel Luar Daerah
                                        ...
                                        $luarDaerahCategories = $luarDaerahs->groupBy('kategori')->map->count(); // Kategori Luar Daerah
                                    @endphp
                                    {{ $sapaan }},
                                </span>
                                <span
                                ...
                                <div class="max-h-[400px] overflow-y-auto p-4 space-y-3 custom-scrollbar">
                                    @forelse($pendingNotifications as $notif)
                                        @php
                                            ...
                                            $targetTab = (Str::contains($kategori, 'NIK') || Str::contains($kategori, 'AKTIVASI')) ? 'laporan_aktivasi' : 'laporan_sistem';
                                        @endphp
                                        <div @click="tab = '{{ $targetTab }}'; open = false; window.scrollTo({top: 0, behavior: 'smooth'})"
                                            ...
                                            <div
                                                ...
                                                <i
                                                    class="fas {{ (Str::contains($kategori, 'NIK') || Str::contains($kategori, 'AKTIVASI')) ? 'fa-fingerprint' : 'fa-database' }}"></i>
                                            </div>
                                            <div class="flex-1">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4 mb-8">
                    @php
                        $cards = [
                            ['label' => 'User', 'val' => $totalUser, 'color' => 'slate-900', 'icon' => 'fa-users', 'txt' => 'white'],
                            ['label' => 'Kendala SIAK', 'val' => $countSiak, 'color' => 'white', 'icon' => 'fa-database', 'txt' => 'orange-500'],
                            ['label' => 'Aktivasi NIK', 'val' => $countAktivasi, 'color' => 'white', 'icon' => 'fa-fingerprint', 'txt' => 'blue-500'],
                            ['label' => 'Proxy', 'val' => $countProxy, 'color' => 'white', 'icon' => 'fa-network-wired', 'txt' => 'emerald-500'],
                            ['label' => 'Pembubuhan', 'val' => $countTte, 'color' => 'white', 'icon' => 'fa-file-signature', 'txt' => 'purple-500'],
                            ['label' => 'Luar Daerah', 'val' => $countLuarDaerah, 'color' => 'white', 'icon' => 'fa-map-marked-alt', 'txt' => 'cyan-500'],
                            ['label' => 'Trouble', 'val' => $countTrouble, 'color' => 'white', 'icon' => 'fa-bug', 'txt' => 'rose-500'],
                        ];
                    @endphp
                    @foreach($cards as $c)
                        <div
                            class="bg-{{ $c['color'] }} p-4 rounded-[2rem] shadow-sm border border-slate-100 relative overflow-hidden">
                            <p class="text-{{ $c['txt'] }} text-[8px] font-black uppercase tracking-widest relative z-10">
                                {{ $c['label'] }}
                            </p>
                            <h2
                                class="text-2xl font-black {{ $c['color'] == 'white' ? 'text-slate-800' : 'text-white' }} leading-tight relative z-10">
                                {{ $c['val'] }}
                            </h2>
                            <i class="fas {{ $c['icon'] }} absolute -right-2 -bottom-2 text-{{ $c['txt'] }}/5 text-5xl"></i>
                        </div>
                    @endforeach
                </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
                    ...
                    <div
                        class="lg:col-span-2 bg-[#1a1f2e] p-8 rounded-[2.5rem] shadow-2xl border border-slate-800 relative overflow-hidden">
                        <div class="relative z-10">
                            <div class="text-center mb-8">
                                <h3
                                    class="font-black text-white text-lg uppercase italic tracking-tighter leading-none">
                                    Buat Pengumuman</h3>
                                <p class="text-[9px] text-slate-400 font-bold uppercase tracking-widest mt-1">Kirim Info
                                    ke Dashboard User</p>
                            </div>

                            @if(session('success'))
                                <div
                                    class="mb-4 p-3 bg-emerald-500/20 border border-emerald-500/50 rounded-xl text-emerald-400 text-[10px] font-bold uppercase text-center italic">
                                    {{ session('success') }}
                                </div>
                            @endif
                            ...
                            @if($errors->any())
                                <div
                                    class="mb-4 p-3 bg-red-500/20 border border-red-500/50 rounded-xl text-red-400 text-[10px] font-bold uppercase text-center italic">
                            ...
                            <form action="{{ route('admin.announcements.store') }}" method="POST" class="space-y-4">
                                ...
                                <div>
                                    <input type="text" name="title" placeholder="Judul Pengumuman..." required
                                        class="w-full bg-[#242b3d] border border-slate-700 rounded-2xl px-6 py-4 text-white text-sm focus:outline-none focus:border-orange-500 transition-all placeholder:text-slate-500 font-medium">
                                </div>
                                <div>
                                    <textarea name="message" rows="4" placeholder="Isi pesan pengumuman..." required
                                        class="w-full bg-[#242b3d] border border-slate-700 rounded-2xl px-6 py-4 text-white text-sm focus:outline-none focus:border-orange-500 transition-all placeholder:text-slate-500 font-medium resize-none"></textarea>
                                </div>

                                <input type="hidden" name="type" value="info">

                                <button type="submit"
                                    class="w-full bg-orange-500 hover:bg-orange-600 text-white font-black uppercase italic py-4 rounded-2xl shadow-lg shadow-orange-900/20 transition-all transform hover:scale-[1.01] active:scale-[0.98] tracking-wider text-sm">
                                    Publikasikan Sekarang
                                </button>
                            </form>
                        </div>
                        <i
                            class="fas fa-bullhorn absolute -right-10 -bottom-10 text-white/5 text-[15rem] -rotate-12"></i>
                    </div>

                    {{-- SEBARAN LAYANAN --}}
                    <div class="bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-100">
                        <div class="mb-6 text-center">
                            <h3 class="font-black text-slate-800 text-xs uppercase italic tracking-tighter">Sebaran
                                Layanan</h3>
                            <p class="text-[8px] text-slate-400 font-bold uppercase">Total Komposisi Masuk</p>
                        </div>
                        ...
                        <div class="mt-6 flex justify-center gap-4">
                            <div class="text-center">
                                <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Total Data
                                </p>
                                <p class="text-lg font-black text-slate-800">
                                    {{ $countSiak + $countAktivasi + $countTte + $countProxy + $countTrouble + $countLuarDaerah }}
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    @foreach($catLists as $list)
                        <div class="bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-100">
                            <h4 class="text-[10px] font-black text-slate-800 uppercase mb-4 flex items-center gap-2">
                                <i class="fas {{ $list['icon'] }} text-{{ $list['color'] }}-500"></i> {{ $list['title'] }}
                            </h4>
                            <div class="space-y-4">
                                @forelse($list['data'] as $name => $count)
                                    <div>
                                        <div class="flex justify-between text-[9px] font-bold mb-1 uppercase text-slate-600">
                                            <span>{{ $name }}</span><span>{{ $count }}</span>
                                        </div>
                                        <div class="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden">
                                            <div class="bg-{{ $list['color'] }}-500 h-full rounded-full"
                                                style="width: {{ ($count / max($list['total'], 1)) * 100 }}%"></div>
                                        </div>
                                    </div>
                                @empty
                                    <p class="text-[9px] text-slate-400 italic">Belum ada data</p>
                                @endforelse
                            </div>
                        </div>
                    @endforeach
            ...
            <script>
                document.addEventListener('DOMContentLoaded', function () {
                    ...
                    new Chart(ctxDonut, {
                        type: 'doughnut',
                        data: {
                            labels: ['SIAK', 'NIK', 'TTE', 'Proxy', 'Trouble', 'Luar Daerah'],
                            datasets: [{
                                data: [
                        {{ $countSiak }},
                        {{ $countAktivasi }},
                        ...
                        {{ $countTrouble }},
                                ...
                                backgroundColor: ['#f97316', '#3b82f6', '#8b5cf6', '#6366f1', '#f43f5e', '#06b6d4'],
                                borderWidth: 0,
                                hoverOffset: 20
                            }]
                        },
                        options: {
                            ...
                            plugins: {
                                legend: {
                                    display: true,
                                    position: 'bottom',
                                    labels: { usePointStyle: true, padding: 20, font: { size: 9, weight: 'bold' } }
                                }
                            }
                        }
                    });
    ...
    editData: { id: '', name: '', email: '', location: '', role: '', pin: '' } 
                ...
                <div
                    ...
                    <form action="{{ route('admin.users.store') }}" method="POST" class="space-y-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                <label class="text-[10px] font-black text-slate-400 uppercase ml-2 block mb-1">NIK (16
                                    Digit)</label>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <p class="text-[9px] text-slate-400 mt-1 ml-2 italic">*Ketik untuk mencari, klik untuk
                                    melihat daftar.</p>
                            </div>
                            <div>
                ...
                <div
                    ...
                    <div
                        ...
                        <h3 class="font-black uppercase tracking-tighter text-slate-700 text-sm italic">Daftar Pengguna
                            Sistem</h3>
                        <span
                ...
                <div x-show="editModal"
                    ...
                    <div @click.away="editModal = false"
                        ...
                        <div class="flex justify-between items-center mb-6 border-b border-slate-100 pb-4">
                            ...
                            <button @click="editModal = false" class="text-slate-400 hover:text-red-500"><i
            ...
            @php
                ...
                $sortedItems = $combined->sort(function ($a, $b) {
                    ...
                    return \Carbon\Carbon::parse($a->created_at)->timestamp <=> \Carbon\Carbon::parse($b->created_at)->timestamp;
                })->values();
                ...
                $jsData = $sortedItems->map(function ($a) {
                    return [
                        ...
                        'date' => \Carbon\Carbon::parse($a->created_at)->format('Y-m-d'),
                        'month' => \Carbon\Carbon::parse($a->created_at)->format('m'),
                        'time' => \Carbon\Carbon::parse($a->created_at)->format('H:i'),
                        'date_human' => \Carbon\Carbon::parse($a->created_at)->translatedFormat('d M Y'),
            ...
            <div x-show="tab === 'laporan_aktivasi'" x-data="{ 
                ...
                <div x-show="showModal"
                    ...
                    x-transition @click.away="showModal = false">
                ...
                <div class="flex flex-wrap items-center gap-4 mb-6">
                    ...
                    {{-- PERBAIKAN LINK EKSPOR EXCEL AKTIVASI --}}
                ...
                <div class="bg-white rounded-[2.5rem] shadow-xl border border-slate-100 overflow-hidden">
                    ...
                    <div class="px-8 py-5 bg-slate-50/50 border-b border-slate-100 flex justify-between items-center">
                        <div class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">
                            Halaman <span x-text="currentPage"></span> Dari <span x-text="totalPages || 1"></span>
                        </div>
                        ...
                        <table class="w-full text-left border-collapse table-fixed min-w-[1100px]">
                            <thead
                                ...
                                <tr>
                                    ...
                                    <th class="p-8 w-1/4">Detail Kendala</th>
                            ...
                            <tbody class="text-sm font-bold text-slate-700 divide-y divide-slate-50">
                                <template x-for="row in pagedData" :key="row.kat_row + row.id">
                                    <tr class="hover:bg-slate-50/50 transition-all">
                                        ...
                                        <td class="p-8">
                                            ...
                                            <template x-if="row.is_restore">
                                                <span
                                                    class="text-slate-400 font-black italic text-[10px]">RESTORE</span>
                                            </template>
                                            <template x-if="!row.is_restore && !row.foto_ktp">
                                                <span
                                        ...
                                        <td class="p-8">
                                            <div class="text-[11px] text-slate-600 font-bold italic mb-3 leading-relaxed bg-slate-50 border border-slate-100 p-4 rounded-2xl line-clamp-2"
                                                :title="row.alasan">
                                                "<span x-text="row.alasan"></span>"
                                            </div>
                                            <div
                        ...
                        (filterKecamatan ? '&kecamatan=' + filterKecamatan : '')"
                            ...
                            <div
                                ...
                                <span
                                    ...
                                    {{ $displayLaporans->currentPage() }} dari {{ $displayLaporans->lastPage() }}</span>
